...
# The backtest module is a normal package import now (src/dynamix/stat.py).
from dynamix import constants as _C
from dynamix import stat as Stat

# Repo root, sourced from constants (used only for output-path fallbacks below).
REPO_ROOT = Path(getattr(_C, "REPO_ROOT", Path(__file__).resolve().parents[3]))
